...
int main(int argc, char **argv) {
    ...
    ComponentMask component_mask = geptil_build_component_mask(components, sizeof(components) / sizeof(components[0]));
    ...
    ComponentMask component_mask1 = geptil_build_component_mask(comp1, sizeof(comp1) / sizeof(comp1[0]));
    u32 archetype_id = geptil_archetype_init(&ctx, component_mask1);


    EntityId entity_id = geptil_entity_init(&ctx, archetype_id, 0);
    geptil_entity_init(&ctx, archetype_id1, 0);
    geptil_entity_init(&ctx, archetype_id1, 0);

    Entity *entity = geptil_get_entity_by_id(&ctx.ecs, entity_id);

    Position *pos = &ctx.ecs.archetypes[entity->archetype_index].positions[entity->archetype_entity_index];

    Archetype *arch = geptil_get_archetype_by_id(&ctx.ecs, entity->archetype_index);

    const ComponentIndex comp2[] = {COMP_POSITION};
    ComponentMask component_mask2 = geptil_build_component_mask(comp2, sizeof(comp2) / sizeof(comp2[0]));
