...
Ecs *init_ecs(Arena *arena, Ecs *ecs)
{
    ...
    ecs->component_masks = arena_alloc(arena, sizeof(uint64_t[256][COMPONENT_MASK_COUNT]));

    ecs->archetypes = (Archetype *)arena_alloc(arena, sizeof(Archetype) * 256);
    ecs->archetype_count = 0;
    ecs->archetype_capacity = 256;

    ecs->query_masks = arena_alloc(arena, sizeof(uint64_t[256][COMPONENT_MASK_COUNT]));
